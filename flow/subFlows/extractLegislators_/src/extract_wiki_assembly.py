...
def main():
    ...
    legislators_file = output_dir / 'wiki_assembly_legislators.json'
    ...
    for legislator in legislators:
        lr = legislator
        if lr['wikiID'][0] != 'Q':
            print(f'missing {lr["year"]}[{lr["constituency_idx"]:3}] {lr["name"]:30} {lr["wiki_url"]}')  # noqa

        if 'wiki_url' not in lr:
            title = lr["wiki_url"].split('/')[-1]
            if title:
                lr['image_url'] = get_image_url(title)
            else:
                lr['image_url'] = ''
    legislators_file.write_text(json.dumps(legislators, indent=2))
