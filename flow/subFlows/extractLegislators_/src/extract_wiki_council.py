...
def fetch_wikidata_id(title):
    BaseURL = 'https://en.wikipedia.org/w/api.php?action=query&prop=pageprops&ppprop=wikibase_item&redirects=1&format=json&titles=' # noqa
...
def get_image_url(title):
    BaseURL = 'https://en.wikipedia.org/w/api.php?action=query&prop=pageimages&format=json&piprop=original&titles=' # noqa
    response = requests.get(f'{BaseURL}{title}')
    response_dict = json.loads(response.text)
    image_url = list(response_dict['query']['pages'].values())[0].get('original', {'source':''})['source'] # noqa
    return image_url
...
def get_html_content(revision_id):
    ...
    # api_url =  "https://en.wikipedia.org/w/index.php?title=Maharashtra_Legislative_Council&oldid=757373235 # noqa
...
def build_members_info(revision_id, revision_date, page_tables):
    def extract_assembly_elected(table):
        ...
        for row in table:
            ...
            member_info['revision_id'] = revision_id
            member_info['revision_date'] = revision_date
    ...
    def extract_constituency_elected(table, table_idx):
        ...
        member_infos = []
        header_row = table.pop(0) # noqa
        constituency_types = ['', 'local_authorities', 'teachers', 'graduates']
        for row in table:
            member_info = dict((k, '') for k in member_fields.split(','))
            member_info['revision_id'] = revision_id
            member_info['revision_date'] = revision_date
            ...
            member_info['elected_type'] = constituency_types[table_idx]
...
def translate_names(members, input_dir):
    en_names = (input_dir / 'names.en.txt').read_text().split('\n')
    mr_names = (input_dir / 'names.mr.txt').read_text().split('\n')
    name_trans_dict = dict((e,m) for (e, m) in zip(en_names, mr_names))
    for member in members:
        member['mr_name'] = name_trans_dict[member['name']]
    return members

def update_wiki_info(members, name_link_dict, input_dir):
    wiki_infos_file, wiki_infos_added = input_dir / 'wiki_infos.json', False
    wiki_infos = json.loads(wiki_infos_file.read_text()) if wiki_infos_file.exists() else {}
    ...
    for member in members:
        ...
        if name_link and 'red_link' not in name_link:
            ...
            if name_title in wiki_infos:
                member['wikiID'], member['image_url'] = wiki_infos[name_title]
            else:
                member['wikiID'] = fetch_wikidata_id(name_title)
                member['image_url'] = get_image_url(name_title)
                wiki_infos[name_title] = (member['wikiID'], member['image_url'])
                wiki_infos_added = True
        else:
            member['wiki_url'] = ''
            member['wikiID'] = ''
            member['image_url'] = ''
    ...
    if wiki_infos_added:
        wiki_infos_file.write_text(json.dumps(wiki_infos))
    return members
...
def merge_members(members):
    def parse_date(date_str):
        ...
        try:
            ...
            if len(date_str) == 4 and date_str[:2] == '20':
                dt = datetime.date(year=dt.year, month=1, day=1)
            return dt
        except ValueError as e:
            print(f'Unable to parse {date_str}')
            return date_str

    def name_rev_date(member):
        return (member['name'], (datetime.date.today() - member['revision_date'].date()))
    
    def name_nospace(member):
        return member['name'].replace(' ', '')

    merged_members = []
    for member in members:
        member['start_date'] = parse_date(member['start_date'])
        member['end_date'] = parse_date(member['end_date'])

    print('#members: {len(members)}')
    sorted_members = sorted(members, key=name_rev_date)
    for key, member_group in groupby(sorted_members, key=name_nospace):
        member_group = list(member_group)
        
        merged_members.append(member_group[0])
        last_uniq_member, num_added = member_group[0], 1
            
        for member in member_group[1:]:
            if member['start_date'] == last_uniq_member['start_date'] and \
               member['end_date'] == last_uniq_member['end_date'] and \
               member['start_date'] and member['end_date']:
                continue
            else:
                merged_members.append(member)
                last_uniq_member = member
                num_added += 1

        print(f'# {key} #{len(member_group)} -> #{num_added}')

    print(f'#{len(members)} -> #{len(merged_members)}')
    for member in merged_members:
        member['revision_date'] = str(member['revision_date'])
...
def main():
    ...
    revisions_file = input_dir / 'revisions.json'
    ...
    revisions_dir = input_dir / 'revisions'
    current_year = -1
    ...
    for revision in revisions:
        ...
        # Only pick the first entry of every year
        if current_year == rev_date.year:
            continue
        current_year = rev_date.year

        revision_id = revision['revid']
        revisions_html_file = revisions_dir / f'{revision_id}.html' 
        if not revisions_html_file.exists():
            revision_html = get_html_content(revision_id)
            revisions_html_file.write_text(revision_html)
            time.sleep(1)            
        else:
            revision_html = revisions_html_file.read_text()
    ...
    members = translate_names(members, input_dir)
    members = update_wiki_info(members, name_link_dict, input_dir)

    members = merge_members(members)

    (output_dir / 'wiki_council_legislators.json').write_text(json.dumps(members))
...
"""
{
      "name": "Aamshya Padavi",
      "party": "SS(UBT)",
      "start_date": "08-Jul-2022",
      "end_date": "07-Jul-2028",
      "elected_type": "assembly_elected",
      "constituency": "",
      "revision_id": 1175628346,
      "revision_date": "2023-09-16 08:37:58",
      "mr_name": "आमश्या पाडवी",
      "wiki_url": "https://en.wikipedia.org/wiki/Aamshya_Padavi",
      "wikiID": "Q112686331",
      "image_url": ""
    },
